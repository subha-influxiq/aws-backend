                    ...
                    if (ts.isCallExpression(decoratorNode) &&
                        ...
                        decoratorNode.arguments.forEach(node => {
                            if (ts.isObjectLiteralExpression(node)) {
                                ...
                                const identifierNodes = importsNode.initializer.elements.filter(node => {
                                        ...
                                        if (node.expression.expression.escapedText === 'RouterModule') {
                                            ...
                                            if (ts.isIdentifier(argument)) {
                                                ...
                                                SourceCodeObj.forEachChild(node => {
                                                    if (ts.isVariableStatement(node) && node.declarationList.declarations[0].name.escapedText === varName) {
    ...
    SourceCodeObj.forEachChild(node => {
        if (ts.isImportDeclaration(node)) {
            node.importClause.namedBindings.forEachChild(name => {
                const identifierIndex = identifiers.indexOf(name.name.escapedText);
